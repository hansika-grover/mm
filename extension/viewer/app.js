...
// local data layer instead of a server (store.js ingests the extension's stored dumps)
const api = async (p) => { try { await window.STORE_READY; return window.MMStore.localApi(p); } catch (e) { console.error(e); return null; } };
let toastT;
...
// ---- boot ------------------------------------------------------------------
window.route = route;                       // let store.js re-render after a new extract
(window.STORE_READY || Promise.resolve()).then(route);
